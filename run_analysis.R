...
# remove the variables without -X or -Y or -Z post fixes (not sure what they are)
# generate new var names using Regex as shown below:


#remove unknown var names (those with no trailing -X,-Y,-Z) NOTE that this also includes the names with 'BodyBody' in them

varNames <- names(finaldf)

found <- grepl("^.*(\\-X|\\-Y|\\-Z)$",names(finaldf)) #find var names with trailing -X or -Y or -Z
found[1:2] <- TRUE #keep the first 2 columns (SubjectID and Activity) 

badNames <- varNames[!found]
badNames #display the removed names (removed columns)

newfinaldf <- finaldf[,found]
...
newNames <- sub("^t","Time.",names(newfinaldf))
...
names(newfinaldf) <- newNames #replace names

#now write out the tidy table to the current working directory
write.table(newfinaldf,"Tidy UCI HAR Dataset.txt", row.names=FALSE)
